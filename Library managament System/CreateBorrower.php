...
if($_SESSION['SUsername'] == NULL) {
     ...
     $SBD = $_SESSION['SUsername'];
if(isset($_GET['SID']) && isset($_GET['SAD'])) {
$SID = $_GET['SID'];
$SAD = $_GET['SAD'];

 if (!empty($SID) && !empty($SAD)) {
   $check = mysql_query("SELECT * FROM student_details WHERE (Borrow_ID = '$SID');");
        if ($check === FALSE)  {
          die(mysql_error());
        }
        $count=mysql_num_rows($check);
          if( $count == 1)
        {
         $result = mysql_query("call CreateBorrower('$SID','$SAD','$SBD')");
    if ($result === FALSE) {
    die(mysql_error());
    }
    header("location: Cupdated.php");
     }  else { echo 'Such a student does not exist';}
...
#content {position:absolute; top:200px; left:575px; margin-right: -1px}
...
<p class="breadcrumb"><a href="AfterLoginF.php">HOME</a> Issue</p>

<div id="content">
<p><form action="CreateBorrower.php" method="GET">
